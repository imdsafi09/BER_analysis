...
%% Output Loop computing Bit-error rate as a function of EbNo

for EbNo = EbNo_vector
    snr = EbNo + 10*log(2);
    AWGN.EbNo = snr;
    numErrs = 0; numBits = 0; results=zeros(3,1);
    %% Inner loop modeling transmitter, channel model and recevier for each EbNo
    while ((numErrs<MaxNumErrs) && (numBits<MaxNumBits))
        ...
        % Recevier 
        y = step(DeModulator, rx_sig); %QPSK Demodualtr
...
grid;title('Eb/No (dB)'); ylabel('BER'); hold;
semilogy(EbNo_vector, theoretical_results, 'dr'); hold;
legend('Simulation', 'Theoritcal')
